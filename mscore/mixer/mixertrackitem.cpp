...
void MixerTrackItem::setCol1AndChannelBind(MixerTrackChannel* col1) { _col1AndChannelBind = col1; }

MixerTrackItem::~MixerTrackItem() {
     if(_col1AndChannelBind) _channel->removeListener(_col1AndChannelBind);
     // need not delete _col1AndChannelBind after setItemWidget
     }

MidiMapping* MixerTrackItem::midiMap() { return _part->masterScore()->midiMapping(_channel->channel()); }
Channel* MixerTrackItem::playbackChannel(const Channel* channel) { return _part->masterScore()->playbackChannel(channel); }
...
QList<Channel*> MixerTrackItem::secondaryPlaybackChannels() {
      ...
      if (_trackType == TrackType::INSTRUMENT) {
...
QList<Channel*> MixerTrackItem::playbackChannels() { return playbackChannels(_part); }
...
bool  MixerTrackItem::getUseDrumset() { return midiMap()->part()->instrument()->useDrumset(); } //Check if drumkit
...
QString MixerTrackItem::getPartName() { return _part->partName(); }
...
void MixerTrackItem::setPartName(QString newName) {
      switch (_trackType) {
            case TrackType::INSTRUMENT:
...
int MixerTrackItem::getVolume() { return (int)(_channel->volume()); }
int MixerTrackItem::getChorus() { return (int)(_channel->chorus()); }
int MixerTrackItem::getReverb() { return (int)(_channel->reverb()); }
int MixerTrackItem::getPan() { return (int)(_channel->pan()) - PANADJUSTMENT; }
bool MixerTrackItem::getMute() { return _channel->mute(); }
bool MixerTrackItem::getSolo() { return _channel->solo(); }
int MixerTrackItem::getMidiChannel() { return _part->masterScore()->midiMapping(_channel->channel())->channel() + 1; }
int MixerTrackItem::getMidiPort() { return _part->masterScore()->midiMapping(_channel->channel())->port() + 1; }
