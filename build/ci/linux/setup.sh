...
case "$PACKARCH" in
  x86_64)
    ...
    qt_version="598" # qt_version="624"
    # qt_revision="r2" # added websocket module
    qt_dir="$BUILD_TOOLS/Qt/${qt_version}"
    if [[ ! -d "${qt_dir}" ]]; then
      mkdir -p "${qt_dir}"
      qt_url="https://s3.amazonaws.com/utils.musescore.org/qt${qt_version}.7z"
  ...
  armv7l | aarch64)
    ...
    # last qt5(515) arm adapted https://github.com/musescore/MuseScore/blob/52d31e741d1fb2c62337ed1772aabc30e96cc9c6/buildscripts/ci/linux/setup-arm.sh
    #
    # !!! PPA !!!
    #
    $SUDO add-apt-repository --yes ppa:beineri/opt-qt-5.12.9-focal
    $SUDO apt-get update
    $SUDO apt-get upgrade -y
    # add an exception for the "detected dubious ownership in repository" (only seen inside a Docker image)
    $SUDO git config --global --add safe.directory /MuseScore
    apt_packages_qt=(
      qt512base
      qt512declarative
      qt512quickcontrols
    ...
    echo export QT_SELECT=qt5 >> ${ENV_FILE} #echo export QT_SELECT=qt6 >> ${ENV_FILE}
    echo export QT_PATH="${qt_dir}" >> ${ENV_FILE}
    echo export QT_PLUGIN_PATH="${qt_dir}/plugins" >> ${ENV_FILE}
    echo export QML2_IMPORT_PATH="${qt_dir}/qml" >> ${ENV_FILE}
    ...
    echo export QMAKE="/usr/bin/qmake5" >> ${ENV_FILE} # echo export QMAKE="/usr/bin/qmake6" >> ${ENV_FILE}
